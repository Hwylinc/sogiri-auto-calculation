            ...
            <div class="card">
                <div class="card-body">
                    @foreach ($diameterDisplayList as $diameter => $id)
                        <a @if($id == $diameter_id) class="active" @endif href="{{ route('calculate.complete',['group_code' => $group_code,'page_tab' => $page_tab,'calculation_id' => $calculation_id,'diameter_id' => $id]) }}">
                            {{ $diameter }}<br>
                        </a>
                    @endforeach
