...
<x-menu select-page="3">
    ...
    <form method="POST" action="{{ route('spare-edit', ['screen' => 'list', 'select_id' => $select_id]) }}">
        @csrf
        <div class="mt-4 flex justify-between">
            <x-message :message="session('message')" />
            @if ($screen === 'list')
                <button type="button" class="p-1 w-[108px] rounded-[64px] bg-[#D9D9D9]" onclick="location.href='{{ route('spare', ['screen' => 'edit', 'select_id' => $select_id]) }}'">編集</button>
            @else
